...
abstract class AttendanceReportsRemoteDataSource {
  ...
  Future<AttendanceReportModel> getDailyReport({required DateTime date, int? employeeId, String? department});
  Future<AttendanceReportModel> getLateReport(AttendanceReportRequestModel request);
  Future<AttendanceReportModel> getEmployeeReport(int employeeId, DateTime fromDate, DateTime toDate);
...
class AttendanceReportsRemoteDataSourceImpl implements AttendanceReportsRemoteDataSource {
  ...
  Future<AttendanceReportModel> getReport(AttendanceReportRequestModel request) async {
    try {
      final response = await dio.get(ApiConstants.attendanceReports, queryParameters: request.toQueryParameters());
      return _fromResponse(response.data);
    } on DioException catch (e) { throw _mapDioError(e); }
  }

  @override
  Future<AttendanceReportModel> getDailyReport({required DateTime date, int? employeeId, String? department}) async {
    try {
  ...
  @override
  Future<AttendanceReportModel> getLateReport(AttendanceReportRequestModel request) async {
    try {
      final response = await dio.get(ApiConstants.attendanceReportLate, queryParameters: request.toQueryParameters());
      return _fromResponse(response.data);
    } on DioException catch (e) { throw _mapDioError(e); }
  }

  @override
  Future<AttendanceReportModel> getEmployeeReport(int employeeId, DateTime fromDate, DateTime toDate) async {
    try {
      final response = await dio.get(
        ApiConstants.attendanceReportEmployee(employeeId),
        queryParameters: {
  ...
  Future<List<AttendanceActionReportModel>> getActions(AttendanceReportRequestModel request) async {
    try {
      final response = await dio.get(ApiConstants.attendanceReportActions, queryParameters: request.toQueryParameters());
  ...
  AttendanceReportModel _fromResponse(dynamic data) {
    if (data is! Map) throw ServerException('Invalid attendance report response');
    return AttendanceReportModel.fromJson(Map<String, dynamic>.from(data));
  }
  ...
  Exception _mapDioError(DioException e) {
    ...
    if (e.type == DioExceptionType.connectionTimeout || e.type == DioExceptionType.connectionError || e.type == DioExceptionType.receiveTimeout) return NetworkException();
    if (status == 404) return NotFoundException('Not found');
    if (status == 403) return ForbiddenException('Forbidden');
    if (status == 400) {
      final data = e.response?.data;
      final message = data is Map ? data['message']?.toString() ?? 'Invalid request' : 'Invalid request';
