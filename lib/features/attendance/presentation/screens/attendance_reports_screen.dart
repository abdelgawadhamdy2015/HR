              ...
              IconButton(
                ...
                onPressed: () async {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('تم تحميل تقرير PDF بنجاح')),
                  );
                },
              ),
            ],
          ...
          body: Column(
            children: [
              _Filters(
                ...
                onLateChanged: (v) => setState(() => _lateOnly = v),
                onSearch: _load,
              ),
              Expanded(
        ...
        Card(
          child: SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: report.employees.map((e) => DataRow(cells: [
                DataCell(Text('${e.fullName}\n${e.code}')),
                DataCell(Text(e.department)),
                DataCell(Text('${e.presentDays}')),
                DataCell(Text('${e.absentDays}')),
                DataCell(Text('${e.lateDays}')),
                DataCell(Text('${e.totalLateMinutes}')),
                DataCell(Text(_minutes(e.totalWorkedMinutes))),
              ])).toList(),
            ),
          ),
        ),
...
class _Stat extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return SizedBox(width: 150, child: Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title), const SizedBox(height: 5), Text(value, style: Theme.of(context).textTheme.headlineSmall)]))));
  }
}
