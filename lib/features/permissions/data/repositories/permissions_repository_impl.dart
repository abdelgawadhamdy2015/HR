...
class PermissionsRepositoryImpl implements PermissionsRepository {
  ...
  Future<Result<List<UserSummary>>> getUsers() async {
    try {
      return Success(await remoteDataSource.getUsers());
    } on NetworkException catch (e) {
      return Error(NetworkFailure(e.message));
    } on ForbiddenException catch (e) {
      return Error(ForbiddenFailure(e.message));
  ...
  Future<Result<void>> assign(
      {required int userId, required int permissionId}) async {
    try {
  ...
  Future<Result<List<Permission>>> updateUserPermissions(
      {required int userId, required List<int> permissionIds}) async {
    try {
      return Success(await remoteDataSource.updateUserPermissions(
          userId: userId, permissionIds: permissionIds));
    } on NetworkException catch (e) {
      return Error(NetworkFailure(e.message));
    } on ValidationException catch (e) {
      return Error(ValidationFailure(e.message));
