...
class _UserPermissionsScreenState extends State<UserPermissionsScreen> {
  ...
  Future<List<_UserItem>> _loadUsers() async {
    ...
    if (_selectedUserId == null && items.isNotEmpty) {
      _selectedUserId = items.first.id;
    }
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'اسم الصلاحية', hintText: 'Example.Manage')),
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider.value(
      value: _cubit,
      child: Scaffold(
        appBar: AppTopBar(title: 'إدارة صلاحيات المستخدمين', actions: [
          if (canManage)
            IconButton(
        ...
        body: FutureBuilder<List<_UserItem>>(
          ...
          builder: (context, usersSnapshot) {
            ...
            if (usersSnapshot.hasError) {
              return Center(
                  child: Text('تعذر تحميل المستخدمين: ${usersSnapshot.error}'));
            }
            final users = usersSnapshot.data ?? const <_UserItem>[];
            if (users.isEmpty) {
              return const Center(
                  child: Text('لا يوجد مستخدمون في قاعدة البيانات'));
            }
              ...
              builder: (context, state) {
                final data = state is PermissionsLoaded
                    ...
                    : state is PermissionActionLoading
                        ? state.data
                        : null;
                log(data?.permissions.length.toString() ?? "");
                if (data == null) {
                  return const Center(
                      child: CircularProgressIndicator(color: AppColors.gold));
                }
                  ...
                  child: ListView(
                    ...
                    children: [
                      DropdownButtonFormField<int>(
                        value: users.any((u) => u.id == _selectedUserId)
                            ? _selectedUserId
                            : null,
                        isExpanded: true,
                        decoration:
                            const InputDecoration(labelText: 'المستخدم'),
                        items: users
                      ...
                      const Text('الصلاحيات الحالية من قاعدة البيانات',
